...
```{r}
p_tile <-
  ggplot(pivot_missing_patterns, aes(x = key, y = fct_rev(id), fill = value)) +
  geom_tile(color = "white") +
  scale_fill_manual(values = c("#BA8AD6", "#bdbdbd", "#636363")) +
  xlab("Variable")  +
...
p_pattern_hist <-
  ggplot(input_pattern_hist, aes(x = fct_rev(id), y = count)) +
...
input_variable_hist <- pivot_missing_patterns %>%
  ...
  group_by(key) %>%
  summarise(variable_count = sum(count)) 
p_variable_hist <-
  ggplot(input_variable_hist, aes(x = key, y = variable_count)) +
  geom_histogram(stat = "identity") +
  scale_y_continuous(labels = waiver()) +
  xlab(element_blank())  +
  ylab("% rows missing")
...
```{r}
p_variable_hist / (p_tile |
                     p_pattern_hist) +  plot_layout(widths = c(3, 1), heights = unit(c(2.5, 7), c('cm', 'null')))
```
...
p_pattern_hist <-
  ggplot(input_pattern_hist, aes(x = fct_rev(id), y = percentage)) +
  geom_histogram(stat = "identity") +
  coord_flip() +
...
p_variable_hist <-
  ggplot(input_variable_hist, aes(x = key, y = percentage)) +
  geom_histogram(stat = "identity") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1L),
                     limits = c(0, 1.0)) +
  xlab(element_blank())  +
...
```{r}
  + plot_spacer() + p_tile + p_pattern_hist + plot_layout(heights = c(2.5, 7), widths = c(4, 1))
```
...
a)  Create a function for creating missing plots based on your code from question 1. It should provide an option to show either missing counts or missing percent. The percent option for `mycars` is shown below.
    ...
    ```{r}
    missing_values_visualization <- function(data, is_count=TRUE) {
      # Çreates dataset with missing 
      ...
      if (statistic == 'count') {
        ...
        p_variable_hist + plot_spacer() + p_tile + p_pattern_hist + plot_layout(widths = c(4, 1))
...
[Craft Beer Analysis](https://kkcp-dsi.github.io/CraftBeerRatingsAnalysis/)

<https://kkcp-dsi.github.io/CraftBeerRatingsAnalysis/>
...
[[https://github.com/[ACCOUNT](https://github.com/%5BACCOUNT](https://github.com/%5BACCOUNT%5D(https://github.com/%5BACCOUNT)%7B.uri%7D){.uri} NAME]/[REPO NAME]/graphs/contributors

[Contributors](https://github.com/kkcp-dsi/CraftBeerRatingsAnalysis/graphs/contributors)

<https://github.com/kkcp-dsi/CraftBeerRatingsAnalysis/graphs/contributors>
